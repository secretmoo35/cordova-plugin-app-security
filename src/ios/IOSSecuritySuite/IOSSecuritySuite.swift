...
public class IOSSecuritySuite {
    ...
    /**
     This type method is used to determine if application is being debugged detect debugging every 2s
    ...
    public static func amIDetectDebuggedAndDeny() {
        Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { timer in DebuggerChecker.denyDebugger() }
    }
    
    @objc func detectDebuggedAndDeny(){
    ...
    */
    ...
    public static func amIDetectReverseEngineeredAndDeny() {
        Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { timer in ReverseEngineeringToolsChecker.denyAmIReverseEngineered() }
    }
    /**
